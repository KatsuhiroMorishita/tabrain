//***********************************************************
//	a3gim2.h -- Arduino Mega Control Library for 3GIM
//
//	History:
...
class A3GS
{
  public:
	...
	int end(void);
	int restart(int pin = 0);   //--@R3.0 change parameter type
	int start(int pin = 0);     //--@R3.0 change parameter type
	int shutdown(void);
	...
	int onSMSReceived(void (*handler)(void));  //--@R3.0 nop
